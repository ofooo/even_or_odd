...
from primes import data_reader
...
def train():
    config = Config()
    model = Model(config)
    trainset = data_reader.DivisibleDataSet(base=17)
    traindata = data_reader.DataLoader(trainset, batch_size=config.batch_size, shuffle=True)
    testset = data_reader.DivisibleDataSet(base=17, dtype='test')
    ...
    for epoch in range(config.max_epoch):
        for i, (x, gold) in enumerate(traindata):
            ...
            if i % config.print_loss == 0 or i == len(traindata) - 1:
                ...
                acc = (gold == pred).sum()
                print(f'epoch={epoch}  i={i}  loss={loss.item():0.4f}  训练集acc={acc.item() / batch_size}  pred={pred}')
